...
class IncidentController extends Controller
{
    public function incidents() : View{
        ...
        if($logedInUser && $logedInUser && $logedInUser->countries){
    ...
    public function gm_service_requests(Request $request,$country) : view{
        ...
        $new_date_from = $session_from_date[1].'/'.$session_from_date[2].'/'.$session_from_date[0];
        $new_date_to = $session_to_date[1].'/'.$session_to_date[2].'/'.$session_to_date[0];
        $projects = incident::countryProjects($country);
        $all_kpi = Incident::allKpiTotal($from_date,$to_date,$country,$projects);
        $country_kpi = Incident::allCountriesTotalCases($from_date,$to_date,$country,$projects,'gm_view');
        $case_types = Incident::countryCaseTypes($from_date,$to_date,$country,$projects);
    ...
    public function handle_ajax_request(){
       ...
       $projects        = isset($_GET['projects']) ? array_filter(explode(',',$_GET['projects'])) : array();    
       ...
       if($request_type === 'gm_all_cases'){
        ...
        $all_cases = Incident::getAllCasetypeByCountry($case_country,$date_from,$date_to,$projects,$start,$length,$sortColumn,$sortDirection);
        ...
        if(array_key_exists('data',$all_cases)){
            ...
            foreach($all_cases['data'] as $key => $case){
                ...
                if($case['Project']){
                    $country_logo = str_replace(' ','-',strtolower($case['Project'])).'.png';
                }
                ...
                $createdon_row .= '<p class="text-secondary small">'.date('H:i',strtotime($case['Created_On'].' + 4 hours')).'</p>';
                $failertime_row = '<p class="mb-0">'.date('D, d M Y',strtotime($case['Failure_Time'].' + 4 hours')).'</p>';
                $failertime_row .= '<p class="text-secondary small">'.date('H:i',strtotime($case['Failure_Time'].' + 4 hours')).'</p>';
                $bg_class = 'bg-orange';
                if($case['CaseSLAStatus'] === 'Expired'){
                    $bg_class = 'bg-red';
                }elseif(in_array($case['CaseSLAStatus'],array('In Progress','In Progress - SLA Extended'))){
                    $bg_class = 'bg-orange';
                }elseif($case['CaseSLAStatus'] === 'Succeeded'){
                    $bg_class = 'bg-green';
                }
                $status_row = '<span class="badge badge-sm '.$bg_class.'">'.$case['CaseSLAStatus'].'</span>';
                $nested = array();
                $nested['count_row'] = $count;
                $nested['logo_row'] = $logo_row;
                $nested['project_details_row'] = $project_details_row;
                $nested['department_row'] = $department_row;
                $nested['source_row'] = $source_row;
                $nested['casetype_row'] = $casetype_row;
                $nested['createdon_row'] = $createdon_row;
                $nested['failertime_row'] = $failertime_row;
                $nested['status_row'] = $status_row;
                $nested['case_id'] = $case['Case_ID'];
                $nested['problem_summary'] = $case['Problem_Summary'];
                $nested['priority'] = $case['Priority'];
                $data[] = $nested;
            }  
        }else{
            $data = array();
        ...
        return response()->json($json_data);
       }else if($request_type === 'support_total_cases'){
            if($date_range){    
                list($from_date,$to_date) = explode('_to_',$date_range);
                $from_day = $from_month = $from_year = $to_day = $to_month = $to_year = '';
                if($from_date){
            ...
            $all_cases = Incident::getCasesByCondition($case_type,$case_country,$date_from,$date_to,$projects,$start,$length,$sortColumn,$sortDirection);
            $count = $start;
            $data = array();
            if(array_key_exists('data',$all_cases)){
                foreach($all_cases['data'] as $key => $case){
        ...
        $all_cases = Incident::getCasesByCondition($case_type,$case_country,$date_from,$date_to,array(),$start,$length,$sortColumn,$sortDirection);
        $count = $start;
        $data = array();
        if(array_key_exists('data',$all_cases)){
            foreach($all_cases['data'] as $key => $case){
                $count = $count + 1;
                $country_logo = $case['Country'] == 'United Arab Emirates' ? 'uae.png' :str_replace(' ','-',strtolower($case['Project'])).'.png';
                if($case['Project']){
                    $country_logo = str_replace(' ','-',strtolower($case['Project'])).'.png';
                }
                $logo_row  = '<div class="row align-items-center"><div class="col-auto"><figure class="avatar avatar-50 border mb-0 coverimg rounded">';
                ...
                $casetype_row = '<p class="mb-0">'.$case['Case_Type'].'</p>';
                $casetype_row .= '<p class="text-secondary small">'.$case['Problem_Category_'].'</p>';
                $createdon_row = '<p class="mb-0">'.date('D, d M Y',strtotime($case['Created_On'].' + 4 hours')).'</p>';
                $createdon_row .= '<p class="text-secondary small">'.date('H:i',strtotime($case['Created_On'].' + 4 hours')).'</p>';
                $failertime_row = '<p class="mb-0">'.date('D, d M Y',strtotime($case['Failure_Time'].' + 4 hours')).'</p>';
                $failertime_row .= '<p class="text-secondary small">'.date('H:i',strtotime($case['Failure_Time'].' + 4 hours')).'</p>';
                $bg_class = 'bg-orange';
        ...
        return response()->json($json_data);
            ...
            Session::put('calender_label',$calendar_label);
            ...
            $all_kpi = Incident::allKpiTotal($date_from,$date_to,$case_country,$projects);
            $kpis = array();
            if($all_kpi){
                foreach($all_kpi as $key => $kpi){
                    $index = str_replace(array(' ','-'),array('_','_'),$key);
            ...
            $render['all_kpi'] = $kpis;
            $render['label']   = $calendar_label;
            $country_kpi = Incident::allCountriesTotalCases($date_from,$date_to,$case_country,$projects,'gm_view');
            $case_types = Incident::countryCaseTypes($date_from,$date_to,$case_country,$projects);
            //$all_cases = Incident::getAllCasetypeByCountry($case_country,$date_from,$date_to,$projects);
            $country_projects = Incident::countryProjects($case_country);
            $render['success'] = true;
            $render['load_gm_view'] = true;
            $render['calender_label'] = $calendar_label;
            $render['gm_view_html_country'] = view('partials.gm_country_problem_category',['all_countries' => $country_kpi,'case_types' => $case_types,'projects'=>$country_projects,'selected_projects' => $projects])->render();
       }else if($request_type === 'project_filter_request'){
            if($date_range){    
                list($from_date,$to_date) = explode('_to_',$date_range);
                $from_day = $from_month = $from_year = $to_day = $to_month = $to_year = '';
                if($from_date){
            ...
            $all_cases = Incident::getGmCasesList($case_type,$prob_category,$case_country,$date_from,$date_to,$projects,$start,$length,$sortColumn,$sortDirection);
            $count = $start;
            $data = array();
            if(array_key_exists('data',$all_cases)){
                foreach($all_cases['data'] as $key => $case){
