...
<?php
    ...
    $from_date = $from_date;
    $to_date = $to_date;
    $date_label = Session::has('calender_label') ? Session::get('calender_label') : 'Last 7 Days';
?>
<div class="container-fluid mt-4">
    <div class="row">
        <div class="col-12 col-md-9 col-lg-10 col-xxl-10">
        ...
        <div class="col-12 col-md-3 col-lg-2 col-xxl-2">
            <div class="card border-0 mb-4">
                <div class="card-body">
                    <h6 class="title">SUPPORT CASES DATE RANGE</h4>
                    <div class="row align-items-center">
                        <div class="col col-sm-auto">
                            <div class="input-group input-group-md">
                                ...
                                <input type="hidden" class="form-control bg-non px-0" value="{{ $date_label }}" id="calendarLabel" />
    ...
    <div class="row ajax_service_requests">
        @include('partials.gm_country_problem_category',['all_countries' => $country_kpi,$case_types,'projects' => $projects])
    </div>
    <div class="row">
...
@section('scripts')
        ...
        $("#titlecalendar").daterangepicker(
            ...
            function (start, end, label) {
                ...
                $.ajax({
                    ...
                    var kpi_failed =
                        $response.all_kpi.Expired > 0
                            ? parseInt($response.all_kpi.Expired)
                            : 0;
                    var near_expiry =
                        $response.all_kpi.Nearing_Expiry > 0
                            ? parseInt($response.all_kpi.Nearing_Expiry)
                            : 0;
		            var near_expiry_sla_extended =
                        $response.all_kpi.Nearing_Expiry___SLA_Extended > 0
                            ? parseInt($response.all_kpi.Nearing_Expiry___SLA_Extended)
                            : 0;
		            var sla_breach = near_expiry+near_expiry_sla_extended;
                    var in_progress_sla =
                        $response.all_kpi.In_Progress > 0
                            ? parseInt($response.all_kpi.In_Progress)
                            : 0;
		            var in_progress_sla_extended =
                        $response.all_kpi.In_Progress___SLA_Extended > 0
                            ? parseInt($response.all_kpi.In_Progress___SLA_Extended)
                            : 0;
		            var in_progress = in_progress_sla+in_progress_sla_extended;
                    var successed =
                    ...
                    if ($response.success) {
                        ...
                        $("span.service_total_cases").html(formatter.format(total_kpi));
                        $("span.service_failed_kpi").html(formatter.format(kpi_failed));
                        $("span.service_nearing_breach_kpi").html(formatter.format(sla_breach));
                        $("span.service_in_progress_kpi").html(formatter.format(in_progress));
                        $("span.service_successed_kpi").html(formatter.format(successed));
    ...
    $("body").on('click','.filter_project_btn',function(){
        let $_projects = [];
        $("input:checkbox[name='projects']:checked").each(function(){
            $_projects.push($(this).val());
        });
        ...
        var $data = {
            ...
            caldender_label: calenderLabel,
            request_type: 'project_filter_request',
            case_country: $case_country,
            projects : $_projects.join(',')
        };
        $.ajax({
